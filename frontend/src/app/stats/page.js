...
function StatCard({ icon: Icon, label, value, sub, color = 'text-rose-400' }) {
  return (
    ...
    <main className="min-h-screen bg-gray-950">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-10">
          <StatCard
            icon={Tv}
            label="Animes totales"
            value={resumen.total_animes || 0}
            color="text-blue-400"
          />
          <StatCard
            icon={TrendingUp}
            label="Episodios vistos"
            value={resumen.total_episodios || 0}
            color="text-purple-400"
          />
          <StatCard
            icon={Clock}
            label="Tiempo invertido"
            value={`${resumen.tiempo_invertido?.dias || 0}d`}
            sub={`${resumen.tiempo_invertido?.horas || 0} horas · ${resumen.tiempo_invertido?.minutos || 0} min`}
            color="text-rose-400"
          />
          <StatCard
            icon={Star}
            label="Puntuación media"
            value={resumen.puntuacion_media || '—'}
            sub={`${resumen.total_puntuados || 0} animes puntuados`}
